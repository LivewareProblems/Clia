...
This is the control script of erlang/elixir releases, first the original in posix shell [as found in elixir](https://github.com/elixir-lang/elixir/blob/main/lib/mix/lib/mix/tasks/release.init.ex#L81-L269)
...
And here is a possible clia version. Note that this is a pure invention. There are no compiler, nor language specification, yet. And this could change at any point. But this is what the syntax could look like.

TODO: implement eval
TODO: define export_release_sys_config
TODO: define globals, like description, version, author, etc

```elixir
#!/bin/clia
# Clia comment use a # at the start of the line
...
@command "start", call: &start. doc: "Starts the system"

@command "start_iex", call: &start_iex, doc: "Starts the system with IEx attached"

@command "daemon", call: &daemon, doc: "Starts the system as a daemon"

@command "daemon_iex", call: &daemon_iex, doc: "Starts the system as a daemon with IEx attached"
...
def init(ctx) do
  ...
  # This means we can call drop the System in System.env()
...
@impl Script.Command
def start(_ctx) do
  do_start("elixir", "--no-halt")
end

@impl Script.Command
def start_iex(_ctx) do
  do_start("iex", "--werl")
end
...
def daemon(_) do
  ...
  do_start ("elixir", ~s[--no-halt --pipe-to "#{sigil_Path("{release_tmp}/pipe")}" "#{sigil_Path("{release_tmp}/log")}"])
end

@impl Script.Command
def daemon_iex(_ctx) do

  release_temp = get_env().release_tmp

  do_start("iex", ~s[--no-halt --pipe-to "#{sigil_Path("{release_tmp}/pipe")}" "#{sigil_Path("{release_tmp}/log")}"])
end
...
def extract_cookie(release_root) do
  ...
  |> String.parse()
with
  FileError(msg) -> panic("extracting cookies had a problem with error #{msg}")
end
